...
def getHistoricalData():
    ...
    counter = 0
    for stockRange in range(len(stock)):
        for x in range(daysBetweenPeriod):
            ...
            stockData = api.get_bars(stock[stockRange], TimeFrame(5, TimeFrameUnit.Minute), start=newDatePeriodStart.isoformat(),
            ...
            for index, row in stockData.iterrows():
                try:
                    ...
                    val = (stock[stockRange], index, row.open*100, row.close*100, row.low*100, row.high*100, row.volume)
                    mycursor.execute(sql, val)
                    db.commit()
                except mysql.connector.errors.IntegrityError:
                    print(stock[stockRange], x)




    print("Done all Requested Data stored locally")
